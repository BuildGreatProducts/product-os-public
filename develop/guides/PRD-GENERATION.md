...
- **Do not duplicate design tokens.** Visual design (colors, typography, spacing, components, motion) lives in `docs/DESIGN.md`, generated separately by `design-design-system`. Reference token names from that file rather than redefining them here. If `docs/DESIGN.md` does not exist, note that the founder should run `design-design-system` before implementation begins.
...
If `docs/DESIGN.md` does not yet exist, add a note at the top of this section: "Visual tokens not yet defined. Run `design-design-system` before implementation begins."
...
Visual design tokens are not a section in the PRD. They live in `docs/DESIGN.md`, generated by `design-design-system`. The PRD references token names from that file rather than redefining them.
