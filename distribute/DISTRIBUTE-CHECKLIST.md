...
Before you start, skim the two reference playbooks you'll lean on throughout: `BONUS - Distribution Channels.md` (which channel to pick and how each one works) and `BONUS - AI Distribution Tools.md` (the software to run them). Set up `BONUS - Measurement & Attribution.md` before Step 3 so every result is a real number.
...
- **What it does:** Turns the plan into evidence. The tracker is a **living document**, not a one-pass step — it's where "what works" accumulates, and it's the input the scaling step reads. Set up measurement first with `BONUS - Measurement & Attribution.md` so every result is a real number, not a guess.
...
- **What it does:** Produces `productos/distribute/4. Scale & Automation Roadmap.md` — for each proven winner, a plan to do more, do better, and automate the process up the maturity ladder, sequenced Now / Next / Later. Reads the winners from `productos/distribute/3. Growth Experiments Tracker.md`.
