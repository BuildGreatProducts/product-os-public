...
Recommended next step after a successful session: run **`studio-design-ux-writing`** (Step 2). It turns the Tone of Voice you just locked into the in-product copy system at `docs/COPY.md` — voice chart, terminology lexicon, and per-surface rules that every later skill and coding agent writes copy from. Then find **one image or website you love** — a screenshot, a marketing site, a reference from the Visual Style section — and run **`studio-design-design-system`** (Step 3). It translates that image, guided by this identity, into `docs/DESIGN.md` + `docs/DESIGN.html` — the colours, fonts, and full token system that everything downstream builds against.
