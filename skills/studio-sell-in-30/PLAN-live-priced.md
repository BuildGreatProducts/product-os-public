...
| 3 | **Checkout verified, or made live** | if Stripe is still in test mode (`sk_test`, `price_test_…`), switch it to live per the payments section of `studio-develop-golive` first; then a real purchase through the live checkout as a stranger would (fresh account, incognito), refunded; fix what breaks | the live receipt; screenshots of landing → paid saved to `docs/` |
