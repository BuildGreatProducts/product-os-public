...
| 3 | **Checkout verified** | a real purchase through the live checkout, as a stranger would (fresh account, incognito), then refunded; fix what breaks | the receipt; the flow from landing to paid in a screen recording |
...
- **Day 3 is a verification, not a setup.** Priced products with no payments are common: a Stripe integration in test mode, a checkout that 404s on mobile, a webhook pointing at a dead URL. Walk it as a stranger. Anything found is fixed the same day.
