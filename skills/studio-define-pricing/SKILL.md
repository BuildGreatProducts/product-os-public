...
Check: buyer is named with a budget bucket and a value shape; one business model with a rejected alternative written down; a billing unit the customer already counts in, with plans that each have a buyer and an entry route with a reason; anchors named with current prices; one launch price with value, cost, and anchor lines and a positive sanity line; the price line is one sentence a stranger would understand. Then hand off: the recommended next step is always the Mini-Launch (`studio-launch`) — within 48 hours, while the number is still warm.
...
- **Park economics questions.** Cost-per-user modelling, margin targets, the moat, the north star — all real, all later. Name where they live (the optional `studio-define-business-strategy` deep dive) and keep moving. The one exception is the cost floor in step 6, which is an order of magnitude, not a model.
