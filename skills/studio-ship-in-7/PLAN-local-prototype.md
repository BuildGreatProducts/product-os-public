...
| --- | --- | --- | --- | --- |
| 1 | **Define backfill + baseline** | `studio-define-from-code` → `studio-define-product` (skip if current); `studio-develop-code-review` over the working tree so the starting state is known | `docs/PRODUCT.md`; the review's verdict line | 2 |
| 2 | **Quality gate** | `studio-develop-security-audit` → do the "Do this right now" items today | `docs/SECURITY-AUDIT.md` verdict; rotated keys if any were committed | 2 |
| 3 | **Fixes** | the build loop over the audit's Fix plan, Critical and High first | Fix plan Critical/High ticked; tests green | 3 |
| 4 | **Deploy guide** | `studio-develop-golive` | `docs/DEPLOY.md`; hosting and service accounts created (🧑 steps) | 1.5 |
| 5 | **Go live** | work `docs/DEPLOY.md` top to bottom | live URL, HTTPS, custom domain if owned | 3 |
| 6 | **Polish** | `studio-design-design-system-from-code` if `docs/DESIGN.md` is missing, then `studio-develop-design-review` over the core flow; fix the inconsistencies it flags | before/after of the core flow; `docs/DESIGN.md` | 2.5 |
| 7 | **Smoke test, announce** | smoke test as a real customer on the live URL (fresh account, incognito); `studio-launch` (stretch) | **smoke test passed**; live post screenshot (stretch) | 2 |
...
- **Polish uses the from-code design skill.** The code already has a de facto design system; `studio-design-design-system-from-code` extracts it and flags inconsistencies, and `studio-develop-design-review` fixes the ones on the core flow. If the member wants a *new* look, that's the AI-generated-app plan, not this one.
