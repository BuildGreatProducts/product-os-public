...
4. **The recommendation.** Once, one paragraph, framed by the outcome, in the closing message and in the Ship Report, not in the Skool post:
   ...
   Then name what to run meanwhile: **`sell-in-30`**, the first-customer challenge, is the next thing to run now the app is live (or the Develop checklist's remaining steps if it isn't).
