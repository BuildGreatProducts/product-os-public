...
**Honest note at enrol:** this is the most ambitious path. `develop-migrate` is deliberately a full inventory-and-verification process because migrations that skip it lose data, break auth, or leave Stripe webhooks pointing at a dead URL. Three sessions is realistic for a small app at a couple of hours each; a large app with many platform-managed services may not fit, and the skill says so at the end of Day 1's inventory. Say this before writing the plan.
