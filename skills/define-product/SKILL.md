...
3. **The Pricing Strategy** — usually `3-Pricing-Strategy.md`. Provides the Business model section: who pays, the business model, the billing unit and plans, the launch price, and the anchors it sits against.
...
5. **The Business Strategy Deep Dive** — `BONUS-Business-Strategy-Deep-Dive.md`, *if the user has done the optional deep dive*. When filled, it upgrades the Business model section (cost & margin), the Why section (unfair advantage), and the Goal section (north star metric). When empty, skip it silently — the Pricing Strategy and launch log carry those sections.

If the Product Offer, Customer Persona, or Pricing Strategy is missing or substantively empty (still has `**Your answer:**` placeholders in most sections), stop and tell the user which one needs to be filled in first, and which sibling Define skill produces it (e.g., *"Customer Persona is mostly empty — run `define-customer-persona` first"*). A synthesis is only as good as the documents it summarises; do not proceed with thin inputs.

**The Mini-Launch is recommended but not blocking.** If it's empty, note that the Proof section has no first-party signal yet, recommend running `mini-launch` (the whole point of the phase's quick win), and proceed if the user wants to. The Business Strategy Deep Dive is optional by design — never block on it or nag about it.
...
**Business model.** A short paragraph on how the product makes money. From the Pricing Strategy: who pays, the business model, the billing unit and plans, the launch price, and the anchors it sits against. If the Business Strategy Deep Dive is filled, add cost-per-customer and margin where strategically interesting (e.g., AI-margin pressure shaping the plan caps) — otherwise the model and one honest launch price, plainly stated, is the correct amount of business model at this stage.
