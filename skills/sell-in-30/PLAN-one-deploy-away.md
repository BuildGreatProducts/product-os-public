...
| 3 | **Messaging alignment + checkout live** | landing page / listing copy from the reviewed offer, shipped; `define-pricing` if `productos/define/3-Pricing-Strategy.md` is missing or unfilled; payments live and a real test purchase, refunded the same session and never counted toward the bar (skip both the pricing and checkout halves if staying free) | before/after of the live hero; the test-purchase receipt |
