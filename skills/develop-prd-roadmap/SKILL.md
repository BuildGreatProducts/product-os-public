...
Recommended next step after a successful session: start the build with the `develop-mvp-build` skill (or run phase-by-phase using the prompt templates in the roadmap's Agent Session Guide) — the repo is already set up: ProductOS lives in `productos/`, the specs are at `docs/`, and the coding-agent guidelines were wired into the repo root at setup. For an existing codebase, run `develop-refactor-plan` to generate `docs/REFACTOR.md`, then `develop-refactor-build` to execute it. The agent builds straight through all phases in one go — there's no need to open a pull request between phases. When you want a quality pass, run your coding agent's built-in review (`/review` in Claude Code, or the built-in review in Cursor or Codex) rather than wiring up a separate review service.
