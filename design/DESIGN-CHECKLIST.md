...
Work top to bottom. Requires `docs/PRODUCT.md` to exist (run the Define phase first — or its fast-track via `studio-define-from-code` if you already have a product). Each step runs a single skill and produces a single file (Step 3 produces a mirrored pair). The order matters — every later skill reads the earlier outputs.
...
- **What to do:** Run `studio-design-ux-writing`.
...
Once `docs/DESIGN.md` and the acquisition surface exist, move to the Develop phase. Open `productos/develop/DEVELOP-CHECKLIST.md`. During the build, `docs/COPY.md` guides every user-facing string; use `studio-develop-design-better` when generating UI, `studio-develop-design-review` before commits, and `studio-develop-cro-audit` once the product has traffic.
